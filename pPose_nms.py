...
def pose_nms(bboxes, bbox_scores, pose_preds, pose_scores):
    ...
    for j in range(len(pick)):
        ...
        # print('bbox merge ', merge_id,  bboxes[merge_id,:].shape)
        final_result.append({
...
def write_json(videofile, all_results, outputpath, for_eval=False):
    '''
    all_result: result dict of predictions
    outputpath: output directory
    '''

    video_name = os.path.basename(videofile).split('.')[0]
    form = opt.format
    json_results = []
    json_results_cmu = {}
    ...
    if form == 'cmu':  # the form of CMU-Pose
        ...
        with open(os.path.join(outputpath, f'Alphapose_{video_name}_results.json'), 'w') as json_file:
